...
public class PortalManager : MonoBehaviour {
	...
	void GenGate () {
		if (this.gameObject.layer != LayerMask.NameToLayer (RecursionMaskLayer))
			this.gameObject.layer = LayerMask.NameToLayer (RecursionMaskLayer);
		...
		if (GateCamObj) {
			...
			if (CurrentCam) {
				...
				GateCamObj.GetComponent<Camera> ().cullingMask &= ~(1 << LayerMask.NameToLayer(RecursionMaskLayer));
		...
		//Apply current portal mesh to the collider
		...
		if (GetComponent<MeshCollider> ().isTrigger != (CurrentCam ? true : false))
			GetComponent<MeshCollider> ().isTrigger = CurrentCam ? (EnableColliderTrigger ? true : false) : false;
	...
	void OnTriggerStay (Collider collision) {
		...
		for (int i = 0; i < CollidedObjs.Length; i++) {
			if (ContinueTriggerEvents [i] && CollidedObjs [i]) {
				...
				ObjCollidedCamObj = CollidedObjs [i].transform.childCount > 0 ? CollidedObjs [i].GetComponentsInChildren<Transform> () [1].gameObject : null;
				...
				if (GateMesh && ProxDetCollidedObjs [i]) {
					...
					if (CollidedObjsAlwaysTeleport [i]) {
						if (!CloneCollidedObjs [i]) {
							...
							ObjCloneCollidedCamObj = CloneCollidedObjs [i].transform.childCount > 0 ? CloneCollidedObjs [i].GetComponentsInChildren<Transform> () [1].gameObject : null;
		...
		if (CollidedObjs.Length > 0) {
			if (!TriggerExit) {
				for (int i = 0; i < CollidedObjs.Length; i++) {
					...
					if (CollidedObjs [i] && CollidedObjs [i].transform.childCount > 0 && CollidedObjs [i].GetComponentsInChildren<Transform> () [1].GetComponent<Camera> () && CollidedObjs [i].GetComponent<UnityStandardAssets.Characters.FirstPerson.FirstPersonController> ()) {
						Camera CollObjCam = CollidedObjs [i].GetComponentsInChildren<Transform> () [1].GetComponent<Camera> ();

						if (!CollObjCam.enabled)
